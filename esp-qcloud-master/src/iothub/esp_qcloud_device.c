...
#define MY_PRODUCT_ID "8HB8WZ2EZI"
#define MY_DEVICE_NAME "FishTankTest"
#define MY_DEVICE_SECRET "0cGCCGO3ywtAB0DkftVtVw=="
...
esp_err_t esp_qcloud_create_device()
{
    ...
    g_device_profile->product_id = MY_PRODUCT_ID;
    g_device_profile->device_name = MY_DEVICE_NAME;
     ...
     */
    g_device_profile->device_secret = MY_DEVICE_SECRET;

    if (strlen(g_device_profile->device_secret) != DEVICE_SECRET_SIZE || strlen(g_device_profile->product_id) != PRODUCT_ID_SIZE)
    {
    ...
    if (strlen(g_device_profile->product_id) != PRODUCT_ID_SIZE || strlen(g_device_profile->cert_crt) == DEVICE_CERT_FILE_DEFAULT_SIZE)
    {
        ESP_LOGE(TAG, "Please check if the authentication information of the device is configured");
        ESP_LOGE(TAG, "Obtain authentication configuration information from login qcloud iothut: ");
        ESP_LOGE(TAG, "https://console.cloud.tencent.com/iotexplorer");
